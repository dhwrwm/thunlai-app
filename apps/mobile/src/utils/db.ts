...
/** Alphabetical browse — glossary entries (English→Bodo) starting with an English letter. */
export async function getGlossaryByLetter(
  ...
  return db.getAllAsync<Word>(
    `SELECT id, bodo, roman, english, source, slug
     FROM words
     WHERE english LIKE ? AND source = 'glossary'
     ORDER BY english
     LIMIT ? OFFSET ?`,
    [`${letter}%`, limit, offset]
  );
...
const STOP_WORDS = new Set([
  ...
  'can', 'not', 'but', 'also', 'used', 'use', 'its', 'into', 'which', 'when',
  'than', 'more', 'one', 'two', 'any', 'all', 'each', 'such', 'being', 'been',
]);

/** Returns words with semantically similar English meanings, excluding the given word. */
export async function getSimilarWords(word: Word, limit = 5): Promise<Word[]> {
  const db = await getDb();
  ...
  const ftsQuery = tokens.map((t) => `english:${t}*`).join(' OR ');
  ...
  try {
    return db.getAllAsync<Word>(
      `SELECT w.id, w.bodo, w.roman, w.english, w.source, w.slug
       FROM words_fts f
       JOIN words w ON w.id = f.rowid
       WHERE words_fts MATCH ? AND w.id != ? AND w.source = ?
       ORDER BY rank
       LIMIT ?`,
      [ftsQuery, word.id, word.source, limit]
    );
  } catch {
    const like = `%${tokens[0]}%`;
    return db.getAllAsync<Word>(
      `SELECT id, bodo, roman, english, source, slug
       FROM words
       WHERE english LIKE ? AND id != ? AND source = ?
       LIMIT ?`,
      [like, word.id, word.source, limit]
    );
  }
}
